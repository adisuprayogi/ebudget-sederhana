    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            @if(auth()->user()->hasAnyRole(['direktur_keuangan', 'direktur_utama', 'kepala_divisi', 'staff_keuangan']))
                <a href="{{ route('refund.select-lpj') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
        ...
        <form method="GET" action="{{ route('refund.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                ...
                @if(request()->filled('search'))
                    <a href="{{ route('refund.index') }}" class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 text-sm font-medium rounded-lg transition-colors">
                        Reset
                    </a>
                @endif
            </div>
            ...
            <div class="bg-white rounded-xl border border-blue-100 overflow-hidden">
                ...
                <div class="md:hidden space-y-3">
                    @foreach($lpjsMenungguRefund as $lpj)
                        <div class="p-3 md:hidden bg-gradient-to-r from-orange-50 to-orange-50/50 border-b border-orange-100">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                <div class="flex-1 min-w-0">
                                    <span class="font-mono text-xs font-semibold text-orange-600 block truncate">{{ $lpj->nomor_lpj }}</span>
                                    <p class="text-sm text-gray-900 truncate mt-0.5">{{ $lpj->judul_lpj ?? '-' }}</p>
                                </div>
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    <span class="text-sm font-semibold text-green-600">{{ formatRupiah($lpj->sisa_dana) }}</span>
                                    <a href="{{ route('refund.create') }}?lpj_id={{ $lpj->id }}" class="inline-flex items-center gap-1 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-medium rounded-lg transition-colors">
                ...
                <div class="bg-orange-50 border-t border-orange-100 px-3 md:px-4 py-2 md:py-3">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        ...
                        <span class="text-base md:text-lg font-bold text-orange-600 truncate">{{ formatRupiah($totalSisaDanaRefund ?? 0) }}</span>
                    </div>
                </div>
            </div>
        @else
