<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-sm mt-0.5">{{ $role->name }}</p>
                </div>
            </div>
    ...
    <div class="max-w-4xl mx-auto">
        <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="bg-gradient-to-r from-amber-50 to-orange-50 px-6 py-4 border-b border-amber-100">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="font-semibold text-amber-900">Edit Role</p>
                        <p class="text-sm text-amber-700 mt-0.5">Ubah data role di bawah ini</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" value="{{ old('name', $role->name) }}" required
                                class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all font-mono {{ $role->name === 'superadmin' ? 'bg-gray-100 cursor-not-allowed' : '' }}"
                                @if($role->name === 'superadmin') readonly @endif>
                        </div>
                        @if($role->name === 'superadmin')
                            <div class="flex items-center gap-2 mt-2 text-amber-600">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                                </svg>
                                <p class="text-sm">Nama role superadmin tidak dapat diubah</p>
                            </div>
                        @endif
                        @error('name')
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <textarea name="description" rows="3"
                                ...
                                placeholder="Deskripsi singkat tentang role...">{{ old('description', $role->description) }}</textarea>
                        </div>
                        @error('description')
                            <p class="text-sm text-red-600 mt-1.5 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                    ...
                    <div>
                        ...
                        @if($role->name === 'superadmin')
                            <div class="p-4 bg-gradient-to-r from-purple-50 to-violet-50 rounded-xl border border-purple-200 flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-mono font-bold text-purple-900 text-lg">*</p>
                                    <p class="text-sm text-purple-700">Full Access (Superadmin memiliki akses penuh)</p>
                                </div>
                            </div>
                        ...
                        @else
                            <div class="relative">
                                ...
                                <textarea name="permissions" rows="5"
                                    ...
                                    placeholder="user.view, user.create, user.edit, user.delete">{{ old('permissions', is_array($role->permissions) ? implode(', ', $role->permissions) : $role->permissions) }}</textarea>
                            </div>
