            ...
            <div class="h-64 flex items-end justify-around space-x-2">
                ...
                @foreach($monthlyTrend ?? [] as $month)
                    <div class="flex flex-col items-center flex-1">
                        <div class="w-full bg-green-500 rounded-t" style="height: {{ ($month['pencairan_total'] / $maxTotal) * 200 }}px; min-height: 4px;"></div>
                        <div class="text-xs text-secondary-500 mt-2">{{ substr($month['month_name'], 0, 3) }}</div>
                        <div class="text-xs font-semibold text-secondary-700">{{ formatRupiah($month['pencairan_total']) }}</div>
                    </div>
                @endforeach
            </div>
        </div>
