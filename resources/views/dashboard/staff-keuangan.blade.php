<x-app-layout>
    ...
    <div class="bg-gradient-to-r from-primary-500 to-primary-600 rounded-2xl shadow-soft p-6 mb-8 text-white">
        <div class="flex items-center justify-between">
            ...
            <div class="text-right">
                <div class="text-sm text-primary-100 mb-1">Total Pagu Perusahaan</div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6 mb-8">
        <div class="bg-gradient-to-br from-green-400 to-green-600 rounded-2xl p-6 text-white shadow-soft">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-green-100 text-sm font-medium">Diproses Hari Ini</p>
                    <p class="text-3xl font-bold mt-1">{{ $data['pencairanProcessedToday'] }}</p>
                    <p class="text-green-100 text-sm mt-2">Pencairan</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-purple-400 to-purple-600 rounded-2xl p-6 text-white shadow-soft">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium">Pagu Terpakai</p>
                    <p class="text-2xl font-bold mt-1">{{ $data['totalPagu'] > 0 ? round(($data['totalTerpakai'] / $data['totalPagu']) * 100, 1) : 0 }}%</p>
                    <p class="text-purple-100 text-sm mt-2">{{ formatRupiah($data['totalTerpakai']) }}</p>
                </div>
                <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl shadow-soft p-6 border-l-4 border-purple-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-purple-600 mt-1">{{ $data['pencairanPending']->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-soft p-6 border-l-4 border-amber-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-amber-600 mt-1">{{ $data['lpjPending']->count() }}</p>
                </div>
    ...
    <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-2xl shadow-soft mb-8 overflow-hidden">
        <div class="px-6 py-4 flex justify-between items-center">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-purple-100 text-sm">{{ $data['pencairanPending']->count() }} pencairan perlu diproses</p>
                </div>
            </div>
            <a href="{{ route('pencairan-dana.index') }}" class="bg-white text-purple-600 px-4 py-2 rounded-xl font-semibold hover:bg-purple-50 transition-colors">
                Proses Sekarang →
            </a>
        </div>
        <div class="bg-white p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($data['pencairanPending']->take(4) as $pencairan)
                    <a href="{{ route('pencairan-dana.show', $pencairan) }}" class="flex justify-between items-center p-4 bg-purple-50 rounded-xl hover:bg-purple-100 transition-colors">
                        <div>
            ...
            @if($data['pencairanPending']->count() > 4)
                <div class="mt-4 text-center">
                    <a href="{{ route('pencairan-dana.index') }}" class="text-purple-600 hover:text-purple-800 text-sm font-medium">
    ...
    <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-2xl shadow-soft mb-8 overflow-hidden">
        <div class="px-6 py-4 flex justify-between items-center flex-wrap gap-4">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-green-100 text-sm">{{ $data['countPengajuanNeedProcessing'] }} pengajuan dengan total {{ formatRupiah($data['totalPengajuanNeedProcessing']) }}</p>
                </div>
            </div>
            <a href="{{ route('pencairan-dana.create') }}" class="bg-white text-green-600 px-6 py-3 rounded-xl font-bold hover:bg-green-50 transition-colors shadow-lg">
        ...
        <div class="bg-white p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($data['pengajuanNeedProcessing']->take(4) as $pengajuan)
                    <a href="{{ route('pencairan-dana.create', ['pengajuan_dana_id' => $pengajuan->id]) }}" class="flex justify-between items-center p-4 bg-green-50 rounded-xl hover:bg-green-100 transition-colors border-2 border-green-200">
                        <div class="flex-1">
                        ...
                        <div class="text-right ml-4">
                            <p class="text-xl font-black text-green-600">{{ formatRupiah($pengajuan->total_pengajuan) }}</p>
                            <p class="text-xs text-green-600 font-medium">{{ \Carbon\Carbon::parse($pengajuan->updated_at)->diffForHumans() }}</p>
                        </div>
                    </a>
                @endforeach
            </div>
            @if($data['pengajuanNeedProcessing']->count() > 4)
                <div class="mt-6 text-center">
                    <a href="{{ route('pengajuan-dana.index', ['status' => 'disetujui']) }}" class="inline-flex items-center px-6 py-3 bg-green-50 text-green-700 rounded-xl font-bold hover:bg-green-100 transition-colors border-2 border-green-300">
            ...
            <div class="bg-gradient-to-r from-amber-500 to-amber-600 px-6 py-4 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-amber-100 text-sm">{{ $data['lpjPending']->count() }} LPJ perlu diverifikasi</p>
                    </div>
                </div>
                <a href="{{ route('lpj.index', ['status' => 'menunggu_verifikasi']) }}" class="bg-white text-amber-600 px-3 py-1 rounded-lg text-sm font-semibold hover:bg-amber-50 transition-colors">
                    Lihat Semua
                </a>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <a href="{{ route('pencatatan-penerimaan.create') }}" class="flex items-center p-6 bg-white rounded-2xl shadow-soft hover:shadow-medium transition-all group">
            ...
            <div class="ml-5">
                <h3 class="text-lg font-bold text-gray-900 group-hover:text-green-600">Pencatatan Penerimaan</h3>
