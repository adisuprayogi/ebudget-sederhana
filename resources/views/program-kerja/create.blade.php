        ...
        <div class="flex items-center justify-between">
            <div>
                <nav class="flex text-sm text-secondary-500 mb-2">
                    ...
                    <span class="text-secondary-900">Tambah Program</span>
                </nav>
                <h1 class="text-2xl font-bold text-secondary-900">Tambah Program Kerja Baru</h1>
                <p class="text-secondary-600 mt-1">Buat program kerja untuk {{ $divisi->nama_divisi }}</p>
            </div>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <form method="POST" action="{{ route('program-kerja.store', $divisi) }}">
            ...
            <div class="bg-white rounded-2xl shadow-soft p-8 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="kode_program" value="{{ old('kode_program') }}" required placeholder="Contoh: 1.01.01" class="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        @error('kode_program')
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nama_program" value="{{ old('nama_program') }}" required placeholder="Contoh: Program Peningkatan Kualitas Pelayanan" class="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        @error('nama_program')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" rows="3" class="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="Jelaskan tujuan dan ruang lingkup program kerja...">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="target_output" value="{{ old('target_output') }}" placeholder="Contoh: 100 kegiatan terlaksana" class="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        @error('target_output')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai') }}" class="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <p class="mt-1 text-xs text-secondary-500">Default: {{ $activePeriode->tanggal_mulai_perencanaan_anggaran->format('d M Y') }}</p>
                        @error('tanggal_mulai')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai') }}" class="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <p class="mt-1 text-xs text-secondary-500">Default: {{ $activePeriode->tanggal_selesai_perencanaan_anggaran->format('d M Y') }}</p>
            ...
            <div class="flex items-center justify-end space-x-4">
                <a href="{{ route('program-kerja.divisi-show', $divisi) }}" class="px-6 py-3 bg-white border border-secondary-300 text-secondary-700 rounded-xl hover:bg-secondary-50 transition-all duration-200">
                    Batal
                </a>
