<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="{{ route('lpj.select-pengajuan') }}" class="p-2 text-secondary-600 hover:text-secondary-900 hover:bg-secondary-100 rounded-lg transition-colors">
    ...
    <div class="max-w-6xl mx-auto py-8">
        @if($pengajuan && $pencairan)
            ...
            <div class="mb-6 bg-white rounded-2xl shadow-soft p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                        ...
                        <p class="font-mono font-semibold text-primary-600">{{ $pengajuan->nomor_pengajuan }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-mono font-semibold text-primary-600">{{ $pencairan->nomor_pencairan }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-secondary-500">Total Pencairan</p>
                        <p class="font-bold text-lg text-primary-600">{{ formatRupiah($pencairan->jumlah_pencairan) }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-medium text-secondary-900">{{ $pencairan->tanggal_pencairan->format('d/m/Y') }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-secondary-500">Metode Pencairan</p>
                        <p class="font-medium text-secondary-900">{{ ucfirst($pencairan->metode_pencairan) }}</p>
                    </div>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('lpj.store') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-soft p-8">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <textarea name="uraian_kegiatan" id="uraian_kegiatan" rows="2"
                                ...
                                placeholder="{{ $pengajuan->judul_pengajuan }}">{{ old('uraian_kegiatan', $pengajuan->judul_pengajuan) }}</textarea>
                ...
                <div class="mb-8">
                    ...
                    <div class="mb-4 bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start">
                        ...
                        <div class="text-amber-700 text-sm">
                            <p class="font-medium">Isi nominal realisasi sesuai penggunaan dana yang sebenarnya.</p>
                            <p class="text-xs mt-1">Total Pencairan: <strong>{{ formatRupiah($pencairan->jumlah_pencairan) }}</strong></p>
                        </div>
                    </div>

                    @if($pencairan->detailPencairans && $pencairan->detailPencairans->count() > 0)
                    <div class="space-y-4">
                        @foreach($pencairan->detailPencairans as $index => $detail)
                        <div class="border border-secondary-200 rounded-xl p-4 bg-secondary-50">
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-secondary-500">
                                        Rencana: {{ $detail->volume }} {{ $detail->satuan }} × {{ formatRupiah($detail->harga_satuan) }} = {{ formatRupiah($detail->subtotal) }}
                                    </p>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                                <input type="hidden" name="details[{{ $index }}][detail_pencairan_id]" value="{{ $detail->id }}">
                                ...
                                <div>
                                    ...
                                    <input type="date" name="details[{{ $index }}][tanggal_realisasi]"
                                        value="{{ old("details.$index.tanggal_realisasi", now()->format('Y-m-d')) }}"
                                        required
                                ...
                                <div>
                                    ...
                                    <input type="number" name="details[{{ $index }}][volume_realisasi]"
                                        value="{{ old("details.$index.volume_realisasi", $detail->volume) }}"
                                        min="0" step="0.01"
                                ...
                                <div>
                                    ...
                                    <input type="text" id="subtotal_{{ $index }}"
                                        value="{{ formatRupiah(old("details.$index.harga_satuan", $detail->harga_satuan) * old("details.$index.volume_realisasi", $detail->volume)) }}"
                                        readonly
                                        class="w-full px-3 py-2 bg-secondary-100 border border-secondary-200 rounded-lg text-secondary-900 font-semibold text-sm">
                                    <input type="hidden" name="details[{{ $index }}][subtotal_realisasi]" id="subtotal_hidden_{{ $index }}">
                                </div>
                            ...
                            <div class="mt-3">
                                ...
                                <textarea name="details[{{ $index }}][keterangan]" rows="2"
                                    class="w-full px-3 py-2 border border-secondary-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm"
                                    placeholder="Keterangan tambahan...">{{ old("details.$index.keterangan") }}</textarea>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    ...
                    <div class="mt-6 p-4 bg-primary-50 rounded-xl border border-primary-200">
                        <div class="flex items-center justify-between">
                            ...
                            <span id="total_realisasi" class="text-xl font-bold text-primary-600">{{ formatRupiah($pencairan->detailPencairans->sum('subtotal')) }}</span>
                        </div>
                        <div class="flex items-center justify-between mt-2">
                            <span class="font-medium text-secondary-900">Sisa Dana:</span>
                            <span id="sisa_dana" class="text-lg font-semibold text-green-600">{{ formatRupiah(0) }}</span>
                        </div>
                        <input type="hidden" name="total_digunakan" id="total_digunakan_hidden">
                        <input type="hidden" name="sisa_dana" id="sisa_dana_hidden">
                    </div>
                    @else
                    <div class="text-center py-8 bg-secondary-50 rounded-xl">
                        <p class="text-secondary-500">Tidak ada rincian pencairan</p>
                    </div>
                    @endif
                </div>
                ...
                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-secondary-200">
                    <a href="{{ route('lpj.select-pengajuan') }}" class="px-6 py-3 border border-secondary-200 text-secondary-700 rounded-xl hover:bg-secondary-50 transition-all duration-200">
                        Batal
                    </a>
        ...
        @else
            ...
            <div class="bg-white rounded-2xl shadow-soft p-12 text-center">
                ...
                <h3 class="text-lg font-semibold text-secondary-900 mb-2">Pengajuan Tidak Ditemukan</h3>
                <p class="text-secondary-500 mb-6">Silakan pilih pengajuan dana terlebih dahulu.</p>
                <a href="{{ route('lpj.select-pengajuan') }}" class="inline-flex items-center px-6 py-3 bg-primary-600 text-white rounded-xl hover:bg-primary-700 transition-all duration-200 shadow-soft hover:shadow-medium">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                    Pilih Pengajuan
    ...
    <script>
    const detailCount = {{ $pencairan->detailPencairans?->count() ?? 0 }};
    const totalPencairan = {{ $pencairan->jumlah_pencairan ?? 0 }};
