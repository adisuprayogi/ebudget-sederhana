<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Edit Rekening Perusahaan</h1>
                    <p class="text-gray-500 text-sm mt-0.5">{{ $rekening->bank->nama_bank }} - {{ $rekening->nomor_rekening_formatted }}</p>
                </div>
            </div>
    ...
    <div class="max-w-4xl mx-auto">
        <form method="POST" action="{{ route('rekening-perusahaan.update', $rekening) }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="bg-gradient-to-r from-teal-50 to-emerald-50 px-6 py-4 border-b border-teal-100">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="font-semibold text-teal-900">Edit Rekening Perusahaan</p>
                        <p class="text-sm text-teal-700 mt-0.5">Ubah data rekening perusahaan di bawah ini</p>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <select name="bank_id" required
                                @class(['w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all appearance-none bg-white', 'border-red-500' => $errors->has('bank_id')])>
                                <option value="">-- Pilih Bank --</option>
                                @foreach($banks as $bank)
                                    <option value="{{ $bank->id }}" {{ old('bank_id', $rekening->bank_id) == $bank->id ? 'selected' : '' }}>
                                        {{ $bank->nama_bank }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="nomor_rekening" value="{{ old('nomor_rekening', $rekening->nomor_rekening) }}"
                                ...
                                @class(['w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all font-mono', 'border-red-500' => $errors->has('nomor_rekening')])>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="atas_nama" value="{{ old('atas_nama', $rekening->atas_nama) }}"
                                placeholder="Contoh: PT Contoh Indonesia" required
                                @class(['w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all', 'border-red-500' => $errors->has('atas_nama')])>
                        </div>
                        @error('atas_nama')
                            <p class="text-sm text-red-600 mt-1.5 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="cabang" value="{{ old('cabang', $rekening->cabang) }}"
                                placeholder="Contoh: Jakarta Pusat"
                                @class(['w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all', 'border-red-500' => $errors->has('cabang')])>
                        </div>
                        @error('cabang')
                            <p class="text-sm text-red-600 mt-1.5 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <select name="mata_uang"
                                @class(['w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all appearance-none bg-white', 'border-red-500' => $errors->has('mata_uang')])>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="saldo_awal" value="{{ old('saldo_awal', $rekening->saldo_awal) }}"
                                step="0.01" min="0" placeholder="0"
                                @class(['w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all font-mono', 'border-red-500' => $errors->has('saldo_awal')])>
                        </div>
                        @error('saldo_awal')
                            <p class="text-sm text-red-600 mt-1.5 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <textarea name="catatan" rows="3"
                                @class(['w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all resize-none', 'border-red-500' => $errors->has('catatan')])
                                placeholder="Catatan tambahan...">{{ old('catatan', $rekening->catatan) }}</textarea>
                        </div>
                        @error('catatan')
                            <p class="text-sm text-red-600 mt-1.5 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                    ...
                    <!-- Jadikan Default -->
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-3 p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border border-amber-200 cursor-pointer hover:from-amber-100 hover:to-orange-100 transition-all">
                            <input type="checkbox" name="is_default" value="1"
                                {{ old('is_default', $rekening->is_default) ? 'checked' : '' }}
                            ...
                            <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center shadow-sm">
                                @if(old('is_default', $rekening->is_default))
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-3 p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl border border-gray-200 cursor-pointer hover:from-gray-100 hover:to-gray-150 transition-all">
                            ...
                            <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center shadow-sm">
                                @if(old('is_active', $rekening->is_active))
