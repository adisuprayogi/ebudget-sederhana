<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Rekening Perusahaan</h1>
                <p class="text-slate-600 mt-1">Perbarui informasi rekening perusahaan</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <form action="{{ route('rekening-perusahaan.update', $rekening) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="cabang" id="cabang"
                           value="{{ old('cabang', $rekening->cabang) }}"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="catatan" id="catatan" rows="3"
                              ...
                              class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('catatan') border-red-500 @enderror">{{ old('catatan', $rekening->catatan) }}</textarea>
                    @error('catatan')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
