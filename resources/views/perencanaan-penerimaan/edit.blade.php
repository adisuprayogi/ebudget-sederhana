<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ route('perencanaan-penerimaan.show', $perencanaan) }}" class="w-10 h-10 rounded-xl bg-blue-50 hover:bg-blue-100 flex items-center justify-center transition-colors">
                    <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </a>
                <div>
                    <h1 class="text-xl font-semibold text-gray-900">Edit Perencanaan Penerimaan</h1>
                    <p class="text-sm text-gray-500 mt-0.5">Ubah data perencanaan dana</p>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl">
        <form method="POST" action="{{ route('perencanaan-penerimaan.update', $perencanaan) }}">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <div class="bg-gray-50 px-8 py-5 border-b border-gray-200">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-xs text-gray-500">Periode Anggaran</p>
                                <p class="text-sm font-medium text-gray-900">{{ $perencanaan->periodeAnggaran->nama_periode ?? '-' }} ({{ $perencanaan->periodeAnggaran->tahun_anggaran ?? '-' }})</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900">{{ $perencanaan->divisi->nama_divisi ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-8 space-y-6">
                    <!-- Kode Rekening & Sumber Dana -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Kode Rekening</label>
                            <input type="text" name="kode_rekening" value="{{ old('kode_rekening', $perencanaan->kode_rekening) }}" placeholder="Contoh: 4.1.1.01" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-mono text-sm">
                            @error('kode_rekening')
                        ...
                        <div>
                            ...
                            <select name="sumber_dana_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all bg-white text-sm">
                    ...
                    <div>
                        ...
                        <textarea name="uraian" rows="3" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none text-sm" placeholder="Jelaskan sumber penerimaan dana...">{{ old('uraian', $perencanaan->uraian) }}</textarea>
                        @error('uraian')
                            <p class="text-sm text-red-600 mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @if(isset($errors) && $errors->has('perkiraan_bulanan'))
                            <p class="text-sm text-red-600 mt-2">{{ $errors->first('perkiraan_bulanan') }}</p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="catatan" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none text-sm" placeholder="Tambahkan catatan atau keterangan tambahan...">{{ old('catatan', $perencanaan->catatan) }}</textarea>
                    </div>
                </div>

                <!-- Actions -->
                <div class="bg-gray-50 px-8 py-4 border-t border-gray-200 flex items-center justify-end gap-3">
                    <a href="{{ route('perencanaan-penerimaan.show', $perencanaan) }}" class="inline-flex items-center gap-2 px-5 py-2.5 text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 font-medium rounded-lg transition-colors text-sm">
                        Batal
                    </a>
    ...
    <script>
        ...
        function renderMonthInputs() {
            ...
            if (availableMonths.length === 0) {
                container.innerHTML = '<div class="col-span-full text-center py-8 text-gray-500 text-sm">Periode anggaran tidak valid</div>';
                return;
            }
