                                    ...
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        @if(auth()->user()->hasAnyRole(['direktur_keuangan', 'staff_keuangan']))
                                            @if(in_array($periode->status, ['draft', 'active']))
                                                <a href="{{ route('periode-anggaran.edit', $periode) }}" class="p-2 text-gray-600 hover:text-amber-600 hover:bg-amber-50 rounded-lg" title="@if($periode->status === 'active') Edit Tanggal @else Edit @endif">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
