...
class UpdatePengajuanDanaRequest extends FormRequest
{
    ...
    {
        $pengajuan = $this->route('pengajuan_dana');

        if (!Auth::check()) {
            return false;
        }

        // User can update their own draft pengajuan
        if ($pengajuan->status === 'draft' && $pengajuan->created_by === Auth::id()) {
            return true;
        }

        // Direktur keuangan can update revisi pengajuan
        if ($pengajuan->status === 'revisi' && Auth::user()->hasPermission('pengajuan_dana.edit_all')) {
            return true;
        }

        return Auth::user()->hasPermission('pengajuan_dana.edit_all');
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'details.*.id' => 'nullable|integer|exists:detail_pengajuans,id',
            ...
            'removed_details.*' => 'integer|exists:detail_pengajuans,id',
            ...
            'removed_attachments.*' => 'integer|exists:attachments,id',
    ...
    /**
     * Validate that pengajuan can be edited
     */
    protected function validateEditable($validator)
    {
        ...
        if (!in_array($pengajuan->status, ['draft', 'revisi'])) {
            $validator->errors()->add('status', 'Pengajuan tidak dapat diedit karena status: ' . $pengajuan->status);
        }
    }
    ...
    protected function validateDetailsSum($validator)
    {
        ...
        if (!empty($details) && $totalPengajuan !== null) {
    ...
    protected function validatePaguAvailability($validator)
    {
        ...
        if (!$divisiId) {
            $divisiId = $this->route('pengajuan_dana')->divisi_id;
        }

        $programKerjaId = $this->input('program_kerja_id');
        if (!$programKerjaId) {
            $programKerjaId = $this->route('pengajuan_dana')->program_kerja_id;
        }
        ...
        $currentPengajuan = $this->route('pengajuan_dana');
        ...
        if ($pagu) {
            // Calculate total approved pengajuan for this program (excluding current one)
            $totalApproved = \App\Models\PengajuanDana::where('divisi_id', $divisiId)
                ->where('program_kerja_id', $programKerjaId)
                ->where('id', '!=', $currentPengajuan->id)
